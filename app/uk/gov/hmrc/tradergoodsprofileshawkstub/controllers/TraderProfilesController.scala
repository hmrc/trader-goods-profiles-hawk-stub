...
class TraderProfilesController @Inject() (
  ...
  private val maintainProfileSchema: Schema =
    schemaValidationService.createSchema("/schemas/tgp-maintain-profile-request-v0.1.json").get
  ...
  def maintainProfile(): Action[RawBuffer] = Action.async(parse.raw) { implicit request =>
    ...
    } yield traderProfileRepository.upsert(body).as {
      Ok(Json.toJson(body))
        .withHeaders(
