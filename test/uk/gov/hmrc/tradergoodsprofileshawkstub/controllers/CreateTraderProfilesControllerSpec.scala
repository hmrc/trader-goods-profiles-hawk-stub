...
class CreateTraderProfilesControllerSpec
    extends AnyFreeSpec
    with Matchers
  ...
  "createProfile" - {
    ...
    val requestBody   = CreateTraderProfileRequest(
      eori = "eori1234567890",
    ...
    "must create the trader profile when given a valid request" in {
      ...
      when(mockRepository.insert(any)).thenReturn(Future.successful(Done))
    ...
    "must not create the trader profile and return an error when there is no correlation-id header" in {
    ...
    "must not create the trader profile and return an error when there is no forwarded-host header" in {
    ...
    "must not create the trader profile and return an error when there is no date header" in {
    ...
    "must not create the trader profile and return an error when there is an invalid date header" in {
    ...
    "must not create the trader profile and return an error when there is no content-type header" in {
    ...
    "must not create the trader profile and return an error when there is an invalid content-type header" in {
    ...
    "must not create the trader profile and return an error when the request body can't be parsed as json" in {
    ...
    "must not create the trader profile and return an error when there are json schema violations" in {
    ...
    "must not create the trader profile and return forbidden with no body when there is no authorization header" in {
    ...
    "must not create the trader profile and return forbidden with no body when there is an invalid authorization header" in {
